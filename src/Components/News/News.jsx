import './../../App.css';
import Command from './../Command.module.css';
import style from './News.module.css';

//const News = () => {
//   return (
//      <div className={Command.container}>
//         News
//      </div>
//   );
//}
//export default News;

const News = () => {
   return (
      <div className={Command.container}>
         <img className={style.main_image} src='https://www.vseznaika.org/wp-content/uploads/2016/03/pic-00892.jpg' />
         <UserCart name='Dmitriy K' dataBirth='2 januar' city='Minsk' education="BSU'11" suite='portfolio' />
         <MyPosts />
      </div>
   );
}
export default News;

const UserCart = (props) => {
   return (
      <div className={style.userCart}>
         <img src='data:image/jpeg;base64,' />
         <div>
            <div className={style.item}><span className={`${style.name} ${style.cartRow}`}>name:</span><span>{props.name}.</span></div>
            <div className={style.item}><span className={`${style.dataBirth} ${style.cartRow}`}>data birth:</span><span>{props.dataBirth}</span></div>
            <div className={style.item}><span className={`${style.city} ${style.cartRow}`}>city:</span><span>{props.city}</span></div>
            <div className={style.item}><span className={`${style.education} ${style.cartRow}`}>education:</span><span>{props.education}</span></div>
            <div className={style.item}><a href='https://developer2594.github.io/portfolio/' className={`${style.suite} ${style.cartRow}`}>suite:</a><span>{props.suite}</span></div>
         </div>
      </div>
   )
}

const MyPosts = () => {
   return (
      <div className={style.myPosts}>
         <div>My Posts</div>
         <input />
         <button>Send</button>
         <Post message='Hey, why nobody love me?' like='1' />
         <Post message='My first post' like='5' />
      </div>
   )
}
const Post = (props) => {
   return (
      <div className={style.post}>
         <img src='data:image/png;base64,' />
         <div>
            <div className={style.postMessage}>{props.message}</div>
            <span>like: {props.like}</span>
         </div>
      </div>
   )
}